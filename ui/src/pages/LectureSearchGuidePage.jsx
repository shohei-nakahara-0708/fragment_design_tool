...
export default function LectureSearchGuidePage() {
  ...
  const addFiles = useCallback(async (rawFiles) => {
    ...
    setError("");
    setResult(null);
    const nextItems = await Promise.all(files.map(buildImageItem));
    setImages((prev) => [...prev, ...nextItems]);
  }, [buildImageItem, generating, loadingRows, rows.length]);
    ...
    <div className="lecture-tool-page">
      <div className="lecture-tool-page__inner">
        <header className="lecture-tool-header">
          <div>
            <h1 className="lecture-tool-header__title">講演会検索案内ツール</h1>
            <div className="lecture-tool-header__sub">
              スプレッドシートの行を選び、アップロード画像からHTML・サムネイル・ZIPを生成します。
            </div>
          </div>
        ...
        <div className="lecture-tool-grid">
          <div className="lecture-tool-panel lecture-tool-panel--wide">
            <div className="lecture-tool-panel__inner">
              <div className="lecture-tool-panel__head">
                <div>
                  <h2 className="lecture-tool-panel__title">画像アップロード</h2>
              ...
              >
                <div className="lecture-tool-drop__title">
                  {sheetReady ? "画像を選択 または ドラッグ&ドロップ" : "参照シート読み込み後に画像をアップロードできます"}
                </div>
                <div className="lecture-tool-drop__sub">
                  {loadingRows ? "シートを読み込み中です。" : "JPG / PNG などを複数アップロードできます。"}
                </div>
                <input
                  ref={fileInputRef}
                  type="file"
          ...
          <div className="lecture-tool-panel lecture-tool-panel--wide">
            <div className="lecture-tool-panel__inner">
              ...
              {!images.length ? (
                <div className="lecture-tool-empty">画像をアップロードすると、ここに選択欄が表示されます。</div>
              ) : (
